...
/// The court + interactive position markers. Shared by every desktop and
/// mobile presentation so the court geometry, marker placement, and
...
/// how the court itself is drawn or how positions are matched.
///
/// [positions] is the player's parsed position codes, primary first (see
/// [parseBasketballPositions]). In view mode this is purely a display; in
/// edit mode tapping a marker reports its code via [onPositionTap] and the
/// caller owns the resulting selection state.
class BasketballCourtCanvas extends StatelessWidget {
  const BasketballCourtCanvas({
    super.key,
  ...
  static const _painter = BasketballCourtPainter();
  ...
  Widget _buildMarker(
    ...
    BasketballPositionMarker marker,
    Rect rect,
    String? primary,
    ...
    final fullName = basketballPositionFullName(l10n, marker.code);

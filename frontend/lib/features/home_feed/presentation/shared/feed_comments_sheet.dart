...
Future<void> showFeedCommentsSheet(
  ...
  return AppSheet.show<void>(
    context: context,
    // A comment list grows; starting tall beats resizing under the reader.
    fullHeight: true,
...
class _FeedCommentsSheetState extends ConsumerState<_FeedCommentsSheet> {
  ...
  Future<void> _delete(VideoComment comment) async {
    try {
      await ref
          .read(feedRepositoryProvider)
          .deleteComment(widget.kind, widget.id, comment.id);
      setState(
  ...
  Widget build(BuildContext context) {
    ...
    // Keyboard inset and drag handle both come from AppSheet now — see the
    // matching note in video_comments_sheet.dart.
    return Column(
      children: [
        Padding(
